...
vec3 processReflection()
{
	vec3 viewDir = normalize(vs_objData.FragPos - u_CameraPos);
	vec3 reflectDir = reflect(viewDir, normalize(vs_objData.Normal));
	return texture(texture_reflect1, vs_objData.TexCoords).rgb * texture(u_Skybox, reflectDir).rgb;
}